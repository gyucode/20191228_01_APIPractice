...
//        어느 서버에 연결할지 서버의 주소( 도메인 - www.naver.com / IP - 192.169.~~ )를 변수로 저장.
...
//            기본주소+기능주소의 조합으로 서버에 찾아갈 주소를 명시.
...
//            서버에 찾아갈 때 들고갈 데이터를 추가.
            ...
            val request = Request.Builder()
                .url(url)
//                    헤더로 담아야 하는 데이터가 있다면 여기에 .header("이름",값)으로 해줘야 함.
                .post(formBody)
                .build()
...
//                    JSON에 대한 상세 분석 or 결과에 따른 처리를 화면(Activity)로 이관.
...
//            Get방식에 맞는 URL생성 => 파라미터가 전부 주소에 노출되도록 작성해야함.
//            Builder => 단계별로 가공해서 완성하는 개념 : Builder
...
//                    GET방식은 제일 기본이 되는 API통신방식. =>메쏘드를 별도로 명시 X
                .build()
            client.newCall(request).enqueue(object :Callback{
                override fun onFailure(call: Call, e: IOException) {
                    Log.d("서버연결실패",e.localizedMessage)
